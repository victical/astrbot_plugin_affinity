...
正向好感不再设置每日上限，会直接累积到真实分数里，形成“好感银行”。可见关系阶段由 `unlocked_stage` 控制，每天刷新时通常最多推进一格；当银行余额达到 `affinity_stage_advance_dynamic_threshold` 时，可在当天推进两格。离线天数不补算；银行分不会丢失，只会在后续实际运行日继续逐日解锁。
...
负向事件使用实时和每日回顾合计的全局单日下限 `-60`，扣分立即生效，并会按真实分数即时下修已解锁阶段。负面事件后 `3` 天内新负面事件默认衰减为 `×0.5`。画像增长仍只由 Engram 的画像 delta 触发，每个新增项 `+10`；LLM 的 `preference_discovered` 信号不单独加分，避免重复计算。LLM 信号去重阈值提升到 `0.8`，不同强度的同类信号不会合并；正向信号最低置信度 `0.60`，负向信号最低置信度 `0.75`。

旧羁绊迁移不再使用固定等级保底，改为在公式分上应用等级倍率：`1 + (old_level - 1) * 5%`，并继续限制最高分 `439`。

`/好感回顾` 默认预览发送指令用户当天的回顾，不写入数据库。也可以指定日期，例如 `/好感回顾 2026-06-05`。管理员可以指定用户与模式：`/好感回顾 <user_id> <date> 写入` 会正式写入回顾事件并更新好感；`/好感回顾 <user_id> <date> 重建` 会备份并重建当天回顾。`预览` 会显示候选事件、LLM 信号计数、预计真实分变化、当前阶段、次日可推进阶段和好感银行余额。
